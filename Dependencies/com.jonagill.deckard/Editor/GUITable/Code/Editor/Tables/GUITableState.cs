...
namespace EditorGUITable
{
	...
	public class GUITableState
	{
		...
		public List<float> absoluteColumnSizes = new List<float> ();

		public List<float> columnRelativeSizes = new List<float> ();
		...
		public void CheckState (List<TableColumn> columns, GUITableEntry tableEntry, float availableWidth)
		{
			...
			if (tableEntry.defaultColumnWidth > 0f)
				columns.ForEach(c => { if (c.entry.defaultWidth == 0f) c.entry.defaultWidth = tableEntry.defaultColumnWidth; });
		...
		public void RightClickMenu (List<TableColumn> columns, Rect? position = null)
		{
			...
			if (rect.Contains (Event.current.mousePosition) && Event.current.type == EventType.MouseDown && Event.current.button == 1)
			{
				...
				for(int i = 0 ; i < columns.Count ; i++)
				{
					...
					{
						int index = i;
						contextMenu.AddItem (new GUIContent (column.entry.title), columnVisible [i], () => columnVisible [index] = !columnVisible [index]);
					}
				}
